...
def update_bullets(screen, stats, sc, inos, bullets):
    """обновлять позиции пуль, перезагружает армию"""
    bullets.update()
...
def gun_kill(stats, screen, sc,  gun, inos, bullets):
    """столкновение пушки и инопланетянов"""
...
def inos_check(stats, screen, sc, gun, inos, bullets):
    """проверка, жобралась ли армия до экрана"""
...
def create_army(screen, inos):
    ...
    ino = Ino(screen)
    ino_widht = ino.rect.width
    number_ino_x = int((1200 - 2 * ino_widht)/ino_widht)
    ...
    for row_number in range(number_ino_y - 1):
        for ino_number in range(number_ino_x):
            ino = Ino(screen)
            ino.x = ino_widht + ino_widht * ino_number
